...
A MAC address is intended to be a unique identifier used as a hardware address. Though this address is unlikely to change, it may be modified for a variety of reasons by software, services, or even you. It should therefore not be relied upon as a unique identifier while still being useful in cases of troubleshooting to sort through packet captures and other investigative measures. When you want to get access to the Internet, your machine needs to have a device called a network interface card. This network adapter has its own hardware address that's used to identify your machine. A MAC address for an Ethernet device looks something like this 00:C4:B5:45:B2:43. MAC addresses are given to network adapters when they are manufactured. Each manufacturer has an organizationally unique identifier (OUI) to identify them as the manufacturer. This OUI is denoted by the first 3 bytes of the MAC address. For example, Dell has 00-14-22, so a network adapter from Dell could have a MAC address like: 00-14-22-34-B2-C2. 
